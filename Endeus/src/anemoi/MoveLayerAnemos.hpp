...
namespace endeus {
	...
	public:
		MoveLayerAnemos(std::string layerId, Vec2f from, Vec2f to, float duration)
		...
		bool update(float dt) override {
			...
			m_current = interp::lerp<Vec2f>(m_from, m_to, t);
		...
		Vec2f getCurrent() const {
			return m_current;
		}

	private:
		std::string m_layerId;
		Vec2f m_from, m_to, m_current;
